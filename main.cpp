...
enum class SimArgT { SA, DA, empty, wrong };
...
auto args_analy(size_t &start_id, size_t &end_id, std::string const &f) {
  ...
  size_t thread_num = std::min(task_num, space::multi_thread::machine_thread_num);

  return std::make_tuple(task_num, thread_num);
}

template <size_t spin_num>
auto call_ode_int(std::string work_dir, ConcurrentFile output, bool is_double_ave, secular::Controler const &ctrl,
...
void single_thread_job(std::string work_dir, ConcurrentFile input, size_t start_id, size_t end_id,
                       ConcurrentFile output, ConcurrentFile log) {
  std::string entry;
  for (; input.execute(get_line, entry);) {
    ...
    if (start_id <= task_id && task_id <= end_id) {
      ...
      if (res == ReturnFlag::max_iter) log << std::to_string(task_id) + ":Max iteration number reaches!\n";
      log.flush();
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  space::tools::read_command_line(argc, argv, user_specified_core_num, input_file_name, start_task_id, end_task_id,
                                  work_dir);
  ...
  timer.start();
  space::multi_thread::multi_thread(thread_num, single_thread_job, work_dir, input_file, start_task_id, end_task_id,
