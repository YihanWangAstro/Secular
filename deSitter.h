...
class deSitter_arg {
 public:
  deSitter_arg(Ctrl const &ctrl, Args const &args, Container const &var) {
    ...
    if (ctrl.Sin_Lin != deS::off) Omega_[0] = args.S1L1() / a_in_eff3_;
    if (ctrl.Sin_Lout != deS::off) Omega_[1] = args.S1L2() / a_out_eff3_;

    if (ctrl.Sin_Lin != deS::off) Omega_[2] = args.S2L1() / a_in_eff3_;
    if (ctrl.Sin_Lout != deS::off) Omega_[3] = args.S2L2() / a_out_eff3_;

    if (ctrl.Sout_Lin != deS::off) Omega_[4] = args.S3L1() / a_out_eff3_;

    if (ctrl.Sout_Lout != deS::off) Omega_[5] = args.S3L2() / a_out_eff3_;

    if (ctrl.Sin_Sin != deS::off) Omega_[6] = args.S1S2() / a_in_eff3_;

    if (ctrl.Sin_Sout != deS::off) Omega_[7] = args.S1S3() / a_out_eff3_;
