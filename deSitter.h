...
auto deSitter_increase(double coef, double a_eff, double v1x, double v1y, double v1z, double v2x, double v2y, double v2z) {
...
inline void deSitter_precession(Args const& args, Container const& var, Container& ddt, double t){
        ...
        if constexpr(LL || SL || spin_num(var) == 3 ) {//if out orbit is coupled, need to calculate a_out_eff
        ...
        if constexpr(spin_num(var) == 1){
            ...
            if constexpr( SL ) {
                auto [ds1x_, ds1y_, ds1z_] = deSitter_increase(args.S_1_L_out_coef, a_out_eff, L2x, L2y, L2z, s1x, s1y, s1z);
        ...
        if constexpr(spin_num(var) == 2) {
            ...
            if constexpr( SL ) {
                auto [ds2x_, ds2y_, ds2z_] = deSitter_increase(args.S_2_L_out_coef, a_out_eff, L2x, L2y, L2z, s2x, s2y, s2z);
        ...
        if constexpr(spin_num(var) == 3) {
            ...
            std::tie(ds3x, ds3y, ds3z) = deSitter_increase(args.S_3_L_out_coef, a_out_eff, L2x, L2y, L2z, s3x, s3y, s3z);
        }
}
