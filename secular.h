...
template<size_t SpinNum>
    struct State_wrapper{
       ...
       // using state_type = State<SpinNum>;
       // using state_wrapper_type = State_wrapper<State<SpinNum>>;
...
namespace boost::numeric::odeint{
    ...
    template<size_t S>
    struct ::boost::numeric::odeint::is_resizeable<State_wrapper<S>>{
        typedef boost::false_type type;
        const static bool value = type::value;
    };

    template <size_t S>
    struct same_size_impl< State_wrapper<S>* , State_wrapper<S>* >
    {
        static bool same_size( const State_wrapper<S>* x , const State_wrapper<S>* y )
        {
    ...
    template<size_t S>
    class State_wrapper_iterator : public boost::iterator_facade< State_wrapper_iterator<S> , double , boost::random_access_traversal_tag> {
public :

    State_wrapper_iterator( void ): m_p(0) , m_stride( 0 ) { }

    explicit State_wrapper_iterator( State_wrapper_iterator *p ) : m_p( p->data ) , m_stride( p->stride ) { }

    friend State_wrapper_iterator end_iterator( State_wrapper_iterator * x){
        State_wrapper_iterator iter( x );
...
private :
    ...
    friend class const_State_wrapper_iterator<S>;

    void increment( void ) { m_p += m_stride; }
    void decrement( void ) { m_p -= m_stride; }
    void advance( ptrdiff_t n ) { m_p += n*m_stride; }
    bool equal( const State_wrapper_iterator &other ) const { return this->m_p == other.m_p; }
    bool equal( const const_State_wrapper_iterator<S> &other ) const{return this->m_p == other.m_p;}
    ...
    template<size_t S>
    class const_State_wrapper_iterator : public boost::iterator_facade< const_State_wrapper_iterator<S> , double , boost::random_access_traversal_tag> {
public :

    const_State_wrapper_iterator( void ): m_p(0) , m_stride( 0 ) { }

    explicit const_State_wrapper_iterator( const_State_wrapper_iterator *p ) : m_p( p->data ) , m_stride( p->stride ) { }

    friend const_State_wrapper_iterator end_iterator( const_State_wrapper_iterator *x ){
        const_State_wrapper_iterator iter( x );
        iter.m_p += iter.m_stride * x->size;
        return iter;
    }
...
private :
    ...
    friend class boost::iterator_core_access;
    friend class State_wrapper_iterator<S>;

    void increment( void ) { m_p += m_stride; }
    void decrement( void ) { m_p -= m_stride; }
    void advance( ptrdiff_t n ) { m_p += n*m_stride; }
    bool equal( const const_State_wrapper_iterator &other ) const { return this->m_p == other.m_p; }
    bool equal( const State_wrapper_iterator &other ) const{return this->m_p == other.m_p;}
    double& dereference( void ) const { return *m_p; }
...
template<size_t S>
    inline State_wrapper_iterator<S> range_begin( State_wrapper_iterator<S> *x )
{
    return State_wrapper_iterator<S>( x );
}

template<size_t S>
inline State_wrapper_iterator<S> range_end( State_wrapper_iterator<S> *x )
{
    return end_iterator( x );
}

template<size_t S>
    inline const_State_wrapper_iterator<S> range_begin( const_State_wrapper_iterator<S> *x )
{
    return const_State_wrapper_iterator<S>( x );
}

template<size_t S>
inline const_State_wrapper_iterator<S> range_end( const_State_wrapper_iterator<S> *x )
{
    return end_iterator( x );
}
}
...
template<size_t SpinNum>
    ...
    public:
        ...
        void operator()(Container const &x, Container& dxdt, double t) {
            ...
            d_args.L1 = coef * L_in * ((c_in * c_in * dn1n2) * cn1n2 - (5 * de1n2) * ce1n2);

            d_args.L2 = - d_args.L1;

            d_args.e1 = coef * c_in * (dn1n2 * ce1n2 + 2 * cn1e1 - 5 * de1n2 * cn1n2);
            
            d_args.e2 = coef * (L_in / L_out) / c_out * ((c_in * c_in * dn1n2) * ce2n1 + (5 * de1n2) * ce1e2 - (0.5 - 3 * de1e1 + 12.5 * de1n2 * de1n2 - 2.5 * c_in * c_in * dn1n2 * dn1n2) * cn2e2);
