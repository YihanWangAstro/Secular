...
#define GR_PROCESS(ACOEF, COEF, num)                                                                           \
  {                                                                                                            \
    double a_eff = calc_a_eff(args.ACOEF, var.L##num##x(), var.L##num##y(), var.L##num##z(), var.e##num##x(),  \
                              var.e##num##y(), var.e##num##z());                                               \
    double Omega = args.COEF / (a_eff * a_eff * a_eff);                                                        \
    dvar.add_e##num(cross_with_coef(Omega, var.L##num##x(), var.L##num##y(), var.L##num##z(), var.e##num##x(), \
                                    var.e##num##y(), var.e##num##z()));                                        \
  }

template <typename Args, typename Container>
inline void GR_precession(bool single_ave, OrbIdx orbit, Args const &args, Container const &var, Container &dvar,
                          double t) {
  if (orbit == OrbIdx::in || orbit == OrbIdx::in_out) {
...
template <typename Args, typename Container>
inline void GW_radiation(OrbIdx orbit, Args const &args, Container const &var, Container &dvar, double t) {
  if (orbit == OrbIdx::in || orbit == OrbIdx::in_out) {
